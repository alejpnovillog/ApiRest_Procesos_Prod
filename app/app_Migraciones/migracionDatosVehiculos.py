...
class DatosVehiculos():
    ...
    def genera_log(self, db, contrib, **data):

        """
        El objetivo es generar un registro en la tabla de log

        :db --------: Es el objeto del manejador de las tablas
        :contrib ---: Es el dominio recibido los valores posibles (dominio o sin valor)
        :data ------: Es el json  que queremos dejar registrado

        :return ----: Es el valor del resultado del insert en la tabla de log

        """
        try:

            # ------------------- cargamos los campos del registros de la tabla log ----------------------------------------
            campos_log = { "apilogerror": json.dumps(data), "apilogtimestamp": datetime.now(), "apilogidcontrib": contrib }
            ...
            error_log = {'errorFuncion': f'Error genero Log {e} '}
            rtnlog = self.genera_log(db, '', **error_log)

        except EOFError as b:
            error_log = {'errorFuncion': f'EOFError {b}'}
            rtnlog = self.genera_log(db, '', **error_log)
